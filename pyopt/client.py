...
class PriceHistory():
    ...
    def __init__(self, symbols: List[str], user_agent: UserAgent, num_months=6) -> None:
        ...
        self.to_date = datetime.today().date() # added
        self.from_date = self.to_date - relativedelta(months=self.num_months) # added
    ...
    def _build_data_frames(self) -> pd.DataFrame:
        ...
        from_date = to_date - relativedelta(months=self.num_months)
    ...
    def _grab_prices(self, symbol: str, from_date: date, to_date: date) -> List[Dict]:
        ...
        if historical_data.ok:
            ...
            for table_row in historical_data:
                ...
                table_row['close'] = float(table_row['close'].replace('$','').replace(',',''))
                table_row['volume'] = int(table_row['volume'].replace(',',''))
                table_row['open'] = float(table_row['open'].replace('$','').replace(',',''))
                table_row['high'] = float(table_row['high'].replace('$','').replace(',',''))
                table_row['low'] = float(table_row['low'].replace('$','').replace(',',''))
